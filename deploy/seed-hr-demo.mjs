/**
 * Run HR demo seed on VPS.
 *   $env:VPS_PASSWORD='...'; node deploy/seed-hr-demo.mjs
 */
...
conn
  .on('ready', async () => {
    console.log('SSH connected — seeding HR demo data...\n');
    try {
      await exec(conn, `cd ${APP_ROOT}/backend && node src/scripts/seedHrDemo.js`);
      console.log('\nSEED_HR_DEMO_OK');
    } catch (e) {
