...
export const LEAD_LIST_ROW_HOVER = 'hover:bg-violet-50/80 transition-colors';

export function leadListRowBg(index) {
  return index % 2 === 0 ? 'bg-slate-50/70' : 'bg-white';
}

export function leadListStickyBg(index) {
  return index % 2 === 0
    ? 'bg-slate-50/70 group-hover:bg-violet-50/80'
    : 'bg-white group-hover:bg-violet-50/80';
}
