...
function Tick({ value, label }) {
  return (
    <div className="inline-flex items-baseline gap-0.5 rounded-md bg-black/20 px-1.5 py-0.5">
...
const Countdown = memo(function Countdown({ expiresAt }) {
...
function AnnouncementHero({ announcement, onView, onParticipate, onDismiss }) {
  if (!announcement) return null;
  const theme = getAnnouncementTheme(announcement.type);
  const progress = announcement.progressPercent ?? null;

  return (
    <section className={cn('relative w-full overflow-hidden rounded-2xl shadow-md', theme.border)}>
      <div className={cn('absolute inset-0 bg-gradient-to-r', theme.gradient)} />
      <div className="absolute inset-0 bg-[linear-gradient(105deg,rgba(255,255,255,0.16)_0%,transparent_42%)]" />
      ...
      <div className="relative flex flex-col gap-2.5 p-3 sm:flex-row sm:items-center sm:gap-4 sm:p-3.5">
        <button
          ...
          className="absolute right-2 top-2 z-10 rounded-full bg-black/15 p-1 text-white/80 hover:bg-black/25"
          aria-label="Dismiss"
        >
          <X className="h-3.5 w-3.5" />
        </button>

        <div className="hidden shrink-0 text-3xl sm:block" aria-hidden>
        ...
        <div className="min-w-0 flex-1 pr-6 text-white">
          ...
          <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
            <h2 className="text-sm font-bold tracking-tight sm:text-[15px]">{announcement.title}</h2>
            {announcement.expiresAt ? <Countdown expiresAt={announcement.expiresAt} /> : null}
          </div>

          <p className="mt-0.5 line-clamp-1 text-[11px] text-white/85 sm:text-xs">{announcement.description}</p>
        ...
        <div className="flex shrink-0 gap-1.5 sm:flex-col lg:flex-row">
          <button
            ...
            className="inline-flex flex-1 items-center justify-center gap-1 rounded-lg border border-white/35 bg-white/15 px-2.5 py-1.5 text-[11px] font-semibold text-white hover:bg-white/25 sm:flex-none"
          >
          ...
          <button
            ...
            className="inline-flex flex-1 items-center justify-center gap-1 rounded-lg bg-amber-300 px-2.5 py-1.5 text-[11px] font-bold text-slate-900 hover:bg-amber-200 sm:flex-none"
          >
...
export default memo(AnnouncementHero);
