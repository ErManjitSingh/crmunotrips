...
const Card = memo(function Card({ item, onReadMore }) {
  const theme = getAnnouncementTheme(item.type);
  return (
    <button
      type="button"
      onClick={() => onReadMore?.(item)}
      className="group relative flex w-[min(68vw,168px)] shrink-0 snap-start items-start gap-2 rounded-xl border border-subtle bg-surface p-2 text-left shadow-sm transition-transform hover:-translate-y-0.5 hover:shadow-md sm:w-[160px]"
    >
      {!item.isRead && <span className="absolute right-1.5 top-1.5 h-1.5 w-1.5 rounded-full bg-violet-500" />}
...
function AnnouncementCarousel({ items = [], onReadMore }) {
  ...
  return (
    <section className="space-y-1.5">
      <div className="flex items-center justify-between">
        ...
        <div className="hidden gap-1 sm:flex">
          <button
            type="button"
            onClick={() => scrollBy(-1)}
            className="rounded-md border border-subtle p-1 hover:bg-surface-elevated"
            aria-label="Prev"
          >
      ...
      >
        {items.map((item) => (
          <Card key={item._id} item={item} onReadMore={onReadMore} />
        ))}
      </div>
    </section>
  );
...
export default memo(AnnouncementCarousel);
