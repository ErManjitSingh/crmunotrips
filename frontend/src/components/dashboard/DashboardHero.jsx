...
export default function DashboardHero({ stats }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-2.5 sm:gap-3 md:grid-cols-4 xl:grid-cols-8">
      {KPI_ITEMS.map((cfg, i) => {
        ...
        return (
          <motion.div
            ...
            className={cn(
              'group relative flex flex-col rounded-2xl border border-subtle bg-surface p-3 shadow-sm sm:items-center sm:p-4 sm:text-center',
              'transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md'
            )}
          >
            <div className="mb-2 flex items-center gap-2.5 sm:mb-3 sm:flex-col sm:gap-0">
              <div
                className={cn(
                  'flex h-9 w-9 shrink-0 items-center justify-center rounded-full shadow-sm ring-4 sm:mb-3 sm:h-11 sm:w-11',
                  cfg.iconBg,
                  cfg.ring
                )}
              >
              ...
              <p className="text-[10px] font-semibold uppercase leading-tight tracking-wider text-content-muted sm:mb-1">
                {cfg.label}
              </p>
            </div>
            <p className="text-lg font-bold tracking-tight text-content-primary metric-tabular leading-none sm:text-xl lg:text-2xl">
              {value}
            </p>
            <div
              className={cn(
                'mt-1.5 inline-flex items-center gap-0.5 text-[10px] font-semibold sm:mt-2 sm:text-[11px]',
            ...
            <p className="mt-0.5 truncate text-[9px] text-content-muted sm:text-[10px]">vs {compareLabel}</p>
          </motion.div>
        );
      })}
    </div>
