...
const cards = [
  ...
  { key: 'hotLeads', label: 'Hot Leads', short: 'Hot', icon: Flame, iconBg: 'bg-orange-100 text-orange-600', path: '/sales-executive/leads/hot' },
  { key: 'monthlyRevenue', label: 'Revenue', short: 'Revenue', icon: IndianRupee, iconBg: 'bg-rose-100 text-rose-600', format: formatCurrency },
];
      ...
      {cards.map(({ key, label, short, icon: Icon, iconBg, format, path }, i) => {
        ...
        return (
          ...
          >
            <Wrapper
              {...(path ? { to: path } : {})}
              className="flex h-full min-h-0 flex-col gap-1 rounded-lg border border-subtle bg-white px-1.5 py-1.5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md dark:bg-slate-900/80 sm:flex-row sm:items-center sm:gap-1.5 sm:px-2 sm:py-2"
            >
